				  ...
				  const struct tcphdr *th, int syn_inerr)
{
	...
	//printk(KERN_EMERG "Pre Check One.\n");
	...
	//printk(KERN_EMERG "Pre Check Two.\n");
	...
	//printk(KERN_EMERG "Pre Check Three.\n");
	...
	//printk(KERN_EMERG "Pre Check Four.\n");
...
struct sock *tcp_v4_hnd_req(struct sock *sk, struct sk_buff *skb)
{
	...
	struct request_sock *req = inet_csk_search_req(sk, &prev, th->source,
						       iph->saddr, iph->daddr);
	if (req) {
		printk(KERN_EMERG "Enter tcp_check_req\n");
			  ...
			  const struct tcphdr *th, unsigned int len)
{
	...
	if (tp->quickopenmd !=0 )
		printk("Entering tcp_rcv_state_process quickopenmd: %d state: %d\n", tp->quickopenmd, sk->sk_state);
	...
	case TCP_LISTEN:
		...
		if (th->syn) {
			...
			//Velicue
			if (tp->quickopenmd == 1) {
				printk(KERN_EMERG "Enter quickopen mode, SYN received\n");
				tp->quickopenmd = 2;
				struct sock *nsk = tcp_v4_hnd_req(sk, skb);
				struct tcp_sock *ntp = tcp_sk(nsk);
				ntp->quickopenmd = 2;
				printk(KERN_EMERG "Return from tcp_v4_hnd_req\n");
				tcp_child_process(sk, nsk, skb);
				printk(KERN_EMERG "Return from tcp_child_process\n");
			}
			 ...
			 */
			kfree_skb(skb);
			printk(KERN_EMERG "You should return 0 here.\n");
			return 0;
		}
		goto discard;
	...
	if (th->ack || tp->quickopenmd == 2) { //Velicue
		...
		switch (sk->sk_state) {
		case TCP_SYN_RECV:
			if (acceptable || tp->quickopenmd == 2) {
				tp->copied_seq = tp->rcv_nxt;
				smp_mb();
				tcp_set_state(sk, TCP_ESTABLISHED);
				sk->sk_state_change(sk);
				...
				if (tp->quickopenmd !=0 )
					printk(KERN_EMERG "State changed! %d", sk->sk_state);
